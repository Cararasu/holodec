...
namespace holoavr {
		...
		"avr", "", 8, 8,
	{
	...
	},
		{
			...
			{ 0, "cf", RegType::eFlag, nullptr, nullptr, 1, 0, false },
			{ 0, "zf", RegType::eFlag, nullptr, nullptr, 1, 1, false },
			{ 0, "nf", RegType::eFlag, nullptr, nullptr, 1, 2, false },
			{ 0, "vf", RegType::eFlag, nullptr, nullptr, 1, 3, false },
			{ 0, "sf", RegType::eFlag, nullptr, nullptr, 1, 4, false },
			{ 0, "hf", RegType::eFlag, nullptr, nullptr, 1, 5, false },
			{ 0, "tf", RegType::eFlag, nullptr, nullptr, 1, 6, false },
			{ 0, "if", RegType::eFlag, nullptr, nullptr, 1, 7, false },
	...
	{ 3, "#seq(=(#t[1],+(#app(#arg[1],#arg[2]),=(#arg[1],#t[1][0,8]),=(#arg[2],#t[1][8,8]),#arg[3])),=($vf,#o),=($cf,#c),=($hf,#a),=($zf,==(#t[1],0)),=($nf,<[s](#t[1],0)),=($sf,#xor($nf,$vf)))" }
	...
	{ AVR_INSTR_ELPM, "elpm",{
		{ 2, "=(#arg[1],#ld($dmem,#app($z,$rampz),#bsize(#arg[1])))" }
	}, InstructionType::eCall },
	{ AVR_INSTR_ELPM | AVR_INSTR_INC_PTR, "elpm+",{
		{ 2, "#seq(=(#arg[1],#ld($dmem,#app($z,$rampz),#bsize(#arg[1]))),=(#t[1],+(#app($z,$rampz),1)),=($z,#t[1][0,16]),=($rampz,#t[1][16,8]))" }
	}, InstructionType::eCall },
		{ AVR_INSTR_EOR, "eor",{
		...
		{ AVR_INSTR_LD, "ld",{
			{ 1, "=(#arg[1],#ld($dmem,#arg[2],#bsize(#arg[1])))" }
		}, InstructionType::eLoad },
		{ AVR_INSTR_LD | AVR_INSTR_INC_PTR, "ld+",{
			{ 1, "#seq(=(#arg[1],#ld($dmem,#arg[2],#bsize(#arg[1]))),=(#arg[2],+(#arg[2],#size(#arg[1]))))" }
		}, InstructionType::eLoad },
		{ AVR_INSTR_LD | AVR_INSTR_DEC_PTR, "ld-",{
			{ 1, "#seq(=(#arg[2],-(#arg[2],#size(#arg[1]))),=(#arg[1],#ld($dmem,#arg[2],#bsize(#arg[1]))))" }
		}, InstructionType::eLoad },
	...
	{ AVR_INSTR_LDS, "lds",{
		{ 1, "=(#arg[1],#ld($dmem,#arg[2],8))" }
	}, InstructionType::eLoad },
	{ AVR_INSTR_LDS, "lpm",{
		{ 1, "=(#arg[1],#ld($pmem,$z,8))" }
	}, InstructionType::eLoad },
	{ AVR_INSTR_LDS | AVR_INSTR_INC_PTR, "lpm+",{
		{ 1, "#seq(=(#arg[1],#ld($pmem,$z,#bsize(#arg[1]))),=($z,+($z,#size(#arg[1]))))" }
	}, InstructionType::eLoad },
	...
	{ AVR_INSTR_MOVW, "movw",{
		{ 1, "#seq(=(#t[1],#app(#arg[3],#arg[4]),=(#arg[1],#t[1][0,8]),=(#arg[2],#t[1][8,8])))" }
	}, InstructionType::eLoad },
	...
	{ AVR_INSTR_POP, "pop",{
		...
		{ AVR_INSTR_RCALL, "rcall",{
			{ 2, "#seq(#push($stack,#ip),#call(#arg[1]))" }
		}, InstructionType::eCall },
		{ AVR_INSTR_RET, "ret",{
			{ 2, "#seq(#pop($stack,#ip),#ret(#pop($stack,#bsize(#ip))))" }
		}, InstructionType::eCall },
		{ AVR_INSTR_RETI, "reti",{
			{ 2, "#seq(#pop($stack,#ip),=($if,1),#ret(#pop($stack,#bsize(#ip))))" }
		}, InstructionType::eCall },
		{ AVR_INSTR_RJMP, "rjmp",{
			{ 2, "#jmp(#arg[1])" }
		}, InstructionType::eCall },
	...
	{ AVR_INSTR_ST, "st",{
		{ 1, "#st($dmem,#arg[1],#arg[2])" }
	}, InstructionType::eLoad },
	{ AVR_INSTR_ST | AVR_INSTR_INC_PTR, "st+",{
		{ 1, "#seq(#st($dmem,#arg[1],#arg[2]),=(#arg[2],+(#arg[2],#size(#arg[1]))))" }
	}, InstructionType::eLoad },
	{ AVR_INSTR_ST | AVR_INSTR_DEC_PTR, "st-",{
		{ 1, "#seq(=(#arg[2],-(#arg[2],#size(#arg[1]))),#st($dmem,#arg[1],#arg[2]))" }
	}, InstructionType::eLoad },

		{ AVR_INSTR_SUB, "sub",{
			{ 3, "#seq(=(#arg[1],-(#arg[1],#arg[2])),=($cf,#c),=($vf,#o),=($hf,#a),=($zf,==(#arg[1],0)),=($nf,<[s](#arg[1],0)),=($sf,#xor($nf,$vf)))" }
		}, InstructionType::eCall },
